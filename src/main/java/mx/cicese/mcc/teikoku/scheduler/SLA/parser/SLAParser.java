...
public class SLAParser {
	...
	final private static Log log = LogFactory.getLog(SLAParser.class);
		...
		try {
			...
			do {
				...
				if(currentLine.startsWith(SLAConstants.PARAMETER_DELIMITER)) {
					this.checkForParameteLine(currentLine.substring(1).trim());
					this.markCurrentPosition();
				} else
				{
	...
	private void checkForParameteLine(String trim) {
		...
		trim.replace("#",""); //clear the number sign
