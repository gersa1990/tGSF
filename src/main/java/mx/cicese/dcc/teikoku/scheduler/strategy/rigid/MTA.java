...
public class MTA implements RStrategy {
	...
	/**
	 * 	Schedules a rigid job to a site based on the MinTurnAround criteria
	 *  (Heuristic #12)For further details on all supported rigid job scheduling 
	...
	public AllocationEntry schedule(Job job) {
		...
		for(UUID s : estmInfo.keySet()) {
			...
			ratio = ( ni != 0 )? (((c - r) / ni) + minTa.get(s).doubleValue())
					: minTa.get(s).doubleValue();
