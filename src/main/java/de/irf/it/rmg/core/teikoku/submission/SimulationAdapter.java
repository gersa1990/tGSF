...
final public class SimulationAdapter
	 ...
	 */
	public JobCompletedEvent submit(Job job)
		...
		Distance endDistance = job.getRuntimeInformation().getRemainingRunTime();
		Instant endInsant = swfJob.getDuration().getAdvent();
			Instant endTime = TimeHelper.add(endInsant,endDistance);
		...
		return jce; // We need to return this event so we have it when need to be removed
	}
}
