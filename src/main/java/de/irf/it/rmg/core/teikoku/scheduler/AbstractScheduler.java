...
abstract public class AbstractScheduler
	...
	/**
	 * LOcal SLA Accepter used by the Strategy
	 */
	private Accepter localAccepter;
	...
	protected void createEvents(Job job, State state) {
		...
		case 3:
			...
			Instant timeRegister = Clock.instance().now();
			Event eventRegister = new JobStartedEventRegister (timeRegister, job, this.getSite(), (JobStartedEvent) event);
			Kernel.getInstance().dispatch(eventRegister);
		...
		case 9:
			...
			event = new SLAReviewEvent(timex, job, this.getSite());
			break;
		} // switch
		Kernel.getInstance().dispatch(event);
	}
	
	//This event is created once all the jobs have been interrupted so the scheduling process can continue
	protected void createJobsInterruptionEvent() {
		Instant timex = Clock.instance().now();
		Event event = new StartSchedulingEvent (timex, this.getSite());
		Kernel.getInstance().dispatch(event);
	}
	...
	public Accepter getLocalAccepter () {
		return this.localAccepter;
	}
	
	
	public final void setLocalAccepter (Accepter accepter) {
			this.localAccepter = accepter;
	...
	public void initialize()
		...
		// load the SLA Strategy accepter
		this.loadLocalSLAAccepter();
	...
	public void initialize2()
		...
		// load the local job queue comparator
		if (this.loadLocalQueueComparator()) {
			// set ascending or descending ordering for the comparator
			this.loadLocalQueueComparatorOrdering();
			this.localQueueComparator.setOrdering(this.localQueueOrdering);
			// Sort the local job queue according the given comparator
			this.getQueue().setOrdering(this.localQueueComparator);
	...
	/*
	 * @author Anuar
	 * added to load the SLA Accepter class
	 */
	private void loadLocalSLAAccepter()
		 	throws InitializationException {
			 StringTokenizer p = new StringTokenizer (this.site.getName(),":");
			 String siteName = p.nextToken();
			 ...
			 String key = ConfigurationHelper
					 .retrieveRelevantKey(
							 c, 
							 siteName, 
							 Constants.CONFIGURATION_SITES_SCHEDULER_LOCALSLAACCEPTER_CLASS);
							 
			 if (key == null) {
				 String msg = "local SLA accepter entry ("
						 + ComputeSite.CONFIGURATION_SECTION
						 + "["
						 + this.site.getName()
						 + "]"
						 + Constants.CONFIGURATION_SITES_SCHEDULER_LOCALSLAACCEPTER_CLASS
						 + ") not found in configuration: using default accepter all jobs are accepted";
				 log.debug(msg);
				 this.setLocalAccepter(null);
				 
			 }
			 else {
				 String className = c.getString(key);
				 try {
					 this.setLocalAccepter(ClassLoaderHelper.loadInterface(className, Accepter.class));
				 ...
				 String msg = "successfully loaded \"" + className
							+ "\" as SLA accpeter for strategy for the scheduler of site \""
							+ this.site.getName() + "\"";
					log.debug(msg);
			 }
		 }
	...
	public void dequeue(Object Caller,Job job) throws DequeuingVetoException{
		if (Caller!=this){
			...
			Site mytestsite = getSite();
			ActivityBroker myactivitybroker = mytestsite.getActivityBroker();
			List<Job> mylist = myactivitybroker.getListOfManagedJobs();
			boolean resp = mylist.remove(job);
			if(!resp) {
