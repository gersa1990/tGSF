...
def CreateMyProcess( commandLine, rect):
    ...
    info = win32process.CreateProcess(
        ...
        "C:\\Users\\Anuar\\workspace\\tGSF\\", # Current directory
        si) # startup info.
...
def RunEm():
    ...
    handle = CreateMyProcess("java -jar -Xmx1400m -Xms900m tGSF.jar -m de.irf.it.rmg.core.teikoku.runtime.SimulationManager -d \"C:\\Users\\Anuar\\workspace\\Experiments\\SSSM-EDD-SLA5\"  -l 0 \"teikoku_1_1.properties\"", rect)
    handles.append(handle)
    # Second instance of notepad will be on the right hand side.
    rect = screenX/2+1, 0, screenX/2, screenY
    handle = CreateMyProcess("java -jar -Xmx1400m -Xms900m tGSF.jar -m de.irf.it.rmg.core.teikoku.runtime.SimulationManager -d \"C:\\Users\\Anuar\\workspace\\Experiments\\SSSM-EDD-SLA5\"  -l 0 \"teikoku_1_2.properties\"", rect)
    handles.append(handle)
    ...
    countdown = range(1,2000)
    countdown.reverse()
    for i in countdown:
        print "Waiting %d seconds for apps to close" % i
        rc = win32event.WaitForMultipleObjects(handles, 1, 1000) # timeout in milli-seconds.
