	...
	*/
	...
	public void decide(Queue<Job> queue, Schedule schedule) {
		if (!queue.isEmpty()) {
			...
			Instant endTime = new Instant(jobToSchedule.getGuaranteedTime()*1000); // use the convert to milliseconds function
			...
			try {
				...
				{
					//The job could not meet its guaranteed time so it is discarded and the job is rejected
					queue.remove();
					String msg = "job " + jobToSchedule.getName() + " was rejected.";
					log.info(msg);
				}
	...
	public JobControlBlock getNextDecision(final Queue<Job> queue, final Schedule schedule){
		...
		if (!queue.isEmpty()) {
			...
			Instant startTime = this.determineRequestedStartTime(jobToSchedule);
			Instant endTime = new Instant(jobToSchedule.getGuaranteedTime()*1000); 
			//Instant endTime = TimeFactory.newEternity();
			Period searchArea = new Period(startTime, endTime);
