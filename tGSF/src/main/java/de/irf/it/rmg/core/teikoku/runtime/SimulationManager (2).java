...
import mx.cicese.mcc.teikoku.scheduler.SLA.generator.sla.TextfileWriter;
...
public class SimulationManager
	...
	final private static Log log2 = LogFactory.getLog("debugger");
	/**
	...
	public void initialize()
		...
		//This part is to create the debugger output
		String strLog = "SITES";
		for (int i = 0; i<workloadSourceNames.length; i++)
			strLog += "\t" + workloadSourceNames[i];
		log2.trace(strLog);
				
		//End of the debugger part
		...
		for (String workloadSourceName : workloadSourceNames) {
			...
			//Create the sla files 
			try {
				site.getSLADistribution().setSWFParser(u);
			} catch (IOException e1) {
				String msg = "configuration error: " + e1.getMessage();
				log.fatal(msg);
				throw new InitializationException(msg);
			}
			URL s = null;
			try {
				boolean createNewSLA = site.getSLADistribution().getMayOverwrite();
				
				s=new URL(url.replaceFirst(".swf", ".sla"));
				if(createNewSLA) 
				{
					TextfileWriter textfileWriter = new TextfileWriter(site.getSLADistribution(),s);
					textfileWriter.initialize();
				}
				else
				{
				 String msg = "No new SLA will be created, existing file is there is one will be used ";
				 log.warn(msg);
				}
				...
				log.fatal(msg);
				throw new InitializationException(msg);
			} catch (TextfileWriterException e1) {
				String msg = "configuration error: " + e1.getMessage();
				log.fatal(msg);
				throw new InitializationException(msg);
			}
			...
			/*
			 * Find the url of the SLA to create the parser
			 */
			...
			WorkloadFilter[] wf = new WorkloadFilter[] {
					...
					//Anuar Lezama B. for SLA simulations this must be set to true otherwise bad things happend :-P
					new RuntimeOveruseFilter(true),
					new IllegalEstimateFilter() };
			workloadSource.initialize(wf);
			
			
			SLAParser slaParser = null;
			
			try {
				 slaParser = new SLAParser(s);
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			
			workloadSource.setSLAParser(slaParser);
			/*
			...
			/*
			 * register workload source and SLA parser in map
			 */
