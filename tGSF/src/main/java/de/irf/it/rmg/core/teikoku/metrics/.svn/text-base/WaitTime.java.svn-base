 ...
 * 
 */
public class WaitTime extends AbstractMetric {
	...
	public void deliverCompletedEvent(JobCompletedEvent event){
		...
		if (event.getTags().containsKey(this.getSite().getUUID().toString())){
			...
			//if (job.getReleasedSite()==this.getSite()){
				this.handleCalculation(event.getCompletedJob(),DateHelper.convertToSeconds(TimeHelper.toLongValue(event.getTimestamp())));
			//}
		}
	}
	
	private void handleCalculation(Job job,long timestamp){
		Object[] values = getLatestValuesPrototype();

		values[0] = timestamp;
		values[1] = job.getName();
		values[2] = this.calculateWaitTime(job);
	...
	/**
	 * Calculates the wait time for the given job.
	 * 
	 * @return The wait time for the given job, in seconds.
	 */
	protected double calculateWaitTime(Job j) {
