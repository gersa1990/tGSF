...
public class MultipleConstantDistributionRestricted  extends AbstractDistribution {
	...
	public MultipleConstantDistributionRestricted() {
	...
	private void parser (String parameters) throws InitializationException 
	{
		...
		for(int i=matches.size()-1; i>=0; i--)
		{
			...
			if(numbers.length ==3)
			{
				...
				constantDistribution.setRestrictedFactor(Double.parseDouble(numbers[2]));
				constantList.add(constantDistribution);
				}
				catch (NumberFormatException e)
				{
					String msg = "There was a problem parsing the input parametrs CM tuples. " + e.toString();
			...
			{
				String msg = "There was a problem parsing the input parametrs CM tuples.";
				throw new InitializationException(msg);
			}
		}//for
	...
	private class ConstantDistribution implements Comparable {
		...
		private double restrictedFactor;
		/**
		...
		void setRestrictedFactor(double restrictedFactor) {
			this.restrictedFactor = restrictedFactor;
		}
		
		double getRestrictedFactor() {
			return this.restrictedFactor;
		}
	...
	public String getHeader() {
		...
		header.append(line1);
		Iterator<MultipleConstantDistributionRestricted.ConstantDistribution> it = constantList.iterator();
		while(it.hasNext())
		{
			header.append("\n"+"# " + it.next().getPrice());
		}
		it = constantList.iterator();
		while(it.hasNext())
		{
			header.append("\n"+"& " + it.next().getRestrictedFactor());
		}
