...
public class SLAMetricsOneWeek extends AbstractMetric {
	...
	private boolean lastJobP = false;
	
	
	
	public SLAMetricsOneWeek() {
		super();
		this.n = 0;
		this.swt = 0;
		this.tni = 0;
	...
	private void handleEvent(Job job, long timestamp) {
		...
		else
		{
			if(!lastJobP)
			{
				...
				double c_diff = c - 604800;
				
				c = 604800; // The completion time is set to the maximum week period
				
				Object[] values = this.getLatestValuesPrototype();
				MetricHelper mh = new MetricHelper((SWFJob)job);
				
				this.n++;
				double p = TimeHelper.toSeconds(job.getRuntimeInformation().getRunTime())- c_diff;
