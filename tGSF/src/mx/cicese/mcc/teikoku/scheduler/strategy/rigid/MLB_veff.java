...
public class MLB_veff implements mx.cicese.dcc.teikoku.scheduler.strategy.rigid.RStrategy {
	...
	public AllocationEntry schedule(Job job) {
		...
		for(UUID s : statInfo.keySet()) {
			...
			double ratio = ((size * p_k)/ (vi*effi*mi)) + this.minLb.get(s).doubleValue();
