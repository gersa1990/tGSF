...
public class MEp implements mx.cicese.dcc.teikoku.scheduler.strategy.rigid.RStrategy {
	...
	public AllocationEntry schedule(Job job) {
		...
		for(UUID s : estmInfo.keySet()) {
			...
			Site site = this.gInfoBroker.getKnownSite(s); 
			
			double ei = site.getSiteEnergyManager().getEnergyConsumption();
