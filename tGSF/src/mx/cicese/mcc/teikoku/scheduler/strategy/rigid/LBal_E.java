...
public class LBal_E implements mx.cicese.dcc.teikoku.scheduler.strategy.rigid.RStrategy {
	/**
	 ...
	 */
	private LinkedList<UUID> knownSites;
	...
	public LBal_E() {
		this.site = null;
		this.knownSites = new LinkedList<UUID>();
	...
	public AllocationEntry schedule(Job job) {
		...
		// Estimate PL for machines where the job can be allocated
		...
		for(UUID sId : this.knownSites) {
			...
			if( size <= m ) {
				...
				//Macadamia
				sum = s.getSiteEnergyManager().getEnergyConsumption();
