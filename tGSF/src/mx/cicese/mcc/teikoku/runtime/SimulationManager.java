 ...
 *   Boston, MA 02110, USA
 */
package mx.cicese.mcc.teikoku.runtime;
...
public class SimulationManager
	...
	public void initialize()
		...
		for (String workloadSourceName : workloadSourceNames) {
			...
			try {
				/*
				 * Aqui se cambia el parser de trabajos
				 */
				//workloadSource = new PrioritySWFParser(u);
	...
	public void commence() {
		/*
		 * loads all jobs from all workloads
		 */
...
*/
		// EnMod
		GridEnergyManager gridEnergyManager = new GridEnergyManager();
		gridEnergyManager.applyEnergyManagementStrategy();
		...
		while (!this.finished) {
			...
			//if lastevent antes que nextRelevant -> liberar replicas
			...
			if (nextRelevantTimestamp == null) {
				/*
				 * the kernel has no more events to handle
				 */
//				gridEnergyManager.getEnergyConsumption();
				this.finished = true;
