...
public class LBal_W implements RStrategy {
	...
	public AllocationEntry schedule(Job job) {
		...
		double jobArea = (((SWFJob)job).getRequestedTime() * ((SWFJob)job).getRequestedNumberOfProcessors());
		...
		for(UUID sId : this.knowSites) {
			...
			if( size <= m ) {
				...
				for(Job j : sj) {
					double s_j = (((SWFJob)j).getRequestedTime() * ((SWFJob)j).getRequestedNumberOfProcessors());
					sum += s_j;
				}
		...
		for(UUID targetSite : ParLoad.keySet()) {
			...
			for(UUID gSite : ParLoad.keySet()) {
				double pl = 0;
				if (targetSite == gSite) 
					pl = (ParLoad.get(gSite).doubleValue() + jobArea) / mSize.get(gSite).doubleValue();
				else
