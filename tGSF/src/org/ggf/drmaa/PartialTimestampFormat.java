 ...
 * <code>CENTURY</code> is set, <code>YEAR</code>, <code>MONTH</code>,
 * and <code>DAY_OF_MONTH</code> must also be set. <code>SECONDS</code> and
...
public class PartialTimestampFormat extends Format {
	 ...
	 * PartialTimestampFormat object since PartialTimestampFormat contains no
	 * state.
	 * 
	 ...
	 * <code>CENTURY</code> is set, <code>YEAR</code>, <code>MONTH</code>,
	 * and <code>DAY_OF_MONTH</code> must also be set. <code>SECONDS</code>
	 * and <code>ZONE_OFFSET</code> are always optional.
	 * </p>
	 * 
	 * @param obj
	 *            the object to format
	 * @param stringBuffer
	 *            the StringBuffer to which to append the results
	 * @param fieldPosition
	 *            ignored
	 ...
	 * In order to represent a PartialTimestamp object as a string, the
	 * PartialTimestamp object cannot have an unset field that is less
	 * significant than the most significant set field. That is to say that if
	 * <code>CENTURY</code> is set, <code>YEAR</code>, <code>MONTH</code>,
	 * and <code>DAY_OF_MONTH</code> must also be set. <code>SECONDS</code>
	 * and <code>ZONE_OFFSET</code> are always optional.
	 * </p>
	 ...
	 * @return the stringBuffer parameter
	 */
	public StringBuffer format(PartialTimestamp obj, StringBuffer stringBuffer,
			FieldPosition fieldPosition) {
		...
		if (obj.isSet(PartialTimestamp.YEAR)) {
			stringBuffer.append(nf.format(obj.get(PartialTimestamp.YEAR)));
			fieldSet = true;
		}
		else if (fieldSet) {
			throw new IllegalArgumentException(
					"In PartialTimestamp object, CENTURY is set but YEAR is not");
		}

		if (obj.isSet(PartialTimestamp.MONTH)) {
			if (fieldSet) {
				stringBuffer.append('/');
			}

			stringBuffer.append(nf.format(obj.get(PartialTimestamp.MONTH) + 1));
			fieldSet = true;
		}
		else if (fieldSet) {
			throw new IllegalArgumentException(
					"In PartialTimestamp object, YEAR is set but MONTH is not");
		}

		if (obj.isSet(PartialTimestamp.DAY_OF_MONTH)) {
			if (fieldSet) {
				stringBuffer.append('/');
			}

			stringBuffer.append(nf.format(obj.get(PartialTimestamp.DAY_OF_MONTH)));
			fieldSet = true;
		}
		else if (fieldSet) {
			throw new IllegalArgumentException(
					"In PartialTimestamp object, MONTH is set but DAY_OF_MONTH is not");
		}

		if (obj.isSet(PartialTimestamp.HOUR_OF_DAY)) {
			if (fieldSet) {
				stringBuffer.append(' ');
			}

			stringBuffer.append(nf.format(obj.get(PartialTimestamp.HOUR_OF_DAY)));
		}
		else {
			throw new IllegalArgumentException(
					"In PartialTimestamp object, HOUR_OF_DAY is not set");
		}

		if (obj.isSet(PartialTimestamp.MINUTE)) {
			stringBuffer.append(':');
			stringBuffer.append(nf.format(obj.get(PartialTimestamp.MINUTE)));
		}
		else {
			throw new IllegalArgumentException(
					"In PartialTimestamp object, MINUTE is not set");
		}

		if (obj.isSet(PartialTimestamp.SECOND)) {
			stringBuffer.append(':');
			stringBuffer.append(nf.format(obj.get(PartialTimestamp.SECOND)));
		}

		if (obj.isSet(PartialTimestamp.ZONE_OFFSET)) {
			int offset = obj.get(PartialTimestamp.ZONE_OFFSET);
	...
	/**
	 * <p>
	 * Translates the PartialTimestamp into a DRMAA specified time string. This
	 * method is equivalent to
	 * <code>format (obj, new StringBuffer (), new FieldPosition (0)).toString
	 * ()</code>.
	 * </p>
	 * <p>
	 * In order to represent a PartialTimestamp object as a string, the
	 ...
	 */
	public PartialTimestamp parse(String str)
			throws ParseException {
	...
	public Object parseObject(String str, ParsePosition parsePosition) {
		...
		char[][] fields = new char[][] { new char[4], new char[2], new char[2],
				new char[2], new char[2], new char[2], new char[1],
		...
		for (int index = parsePosition.getIndex(); index < chars.length; index++) {
			...
			if ((field != 6)
				...
				// System.out.println ("Found digit at " + index + ": fields[" +
				// field + "][" + count + "] = " + chars[index]);
				/*
			...
			/*
			 * If the character is a colon, and we're not in the date, second,
			 * or offset sign fields, and we're at the end of the field, advance
			 * to the next field.
			 */
			...
			/*
			 * If the character is a space, and we're not in the hour, offset
			 * sign, or offset hour, and we're at the end of the field, advance
			 * to the next field.
			 */
				...
				if (!inWhitespace) {
					/*
					 * If we found the hour instead of the month, skip to the
					 * hour.
					 */
					if ((field == 0) && (count == 2)) {
						field = 3;
					...
					/*
					 * If we found the hour instead of the day, skip to the
					 * hour.
					 */
			...
			/*
			 * If the character is a minus or plus and we're in the offset
			 * field, set the offset field.
			 */
			else if ((field == 6)
					&& ((chars[index] == '-') || (chars[index] == '+'))) {
				// System.out.println ("Found " + chars[index] + " at " +
				// index);
			...
			/*
			 * If we're not in a patch of whitespace, we're in the minute,
			 * second, or offset minute, and we're at the end of the field, and
			 * we can't parse the next character, we're done.
			 */
			...
			/*
			 * If we're in a patch of whitespace, and we're in the second or
			 * offset sign, and we can't parse the next character, we're done,
			 * but we have to note the end of the string as being the beginning
			 * of the whitespace.
			 */
			else if (inWhitespace && (count == 0)
					&& ((field == 5) || (field == 6))) {
				// System.out.println ("Found end of date at " +
				// whitespaceIndex);
			...
			/* If we can't parse the next character, we have a problem. */
				...
				// System.out.println ("Found end of parsable string at " +
				// index);
		...
		if (Character.isDigit(fields[3][0])) {
			pt.set(PartialTimestamp.HOUR_OF_DAY, Integer.parseInt(new String(fields[3])));
			...
			if (Character.isDigit(fields[2][0])) {
				pt
						.set(PartialTimestamp.DAY_OF_MONTH, Integer.parseInt(new String(
								fields[2])));
				// System.out.println ("DATE=" + new String (fields[2]));
				pt.set(PartialTimestamp.MONTH, Integer.parseInt(new String(fields[1])) - 1);
				...
				if (year.length() == 4) {
					pt.set(PartialTimestamp.YEAR, Integer.parseInt(year.substring(2, 4)));
				...
				/* If the year has two digits, parse the year. */
					...
					pt.set(PartialTimestamp.YEAR, Integer.parseInt(year));
			 ...
			 */
				...
				if (month.length() != 2) {
					...
					pt.set(PartialTimestamp.MONTH, Integer.parseInt(month) - 1);
			 ...
			 */
				...
				if (day.length() != 2) {
					...
					pt.set(PartialTimestamp.DAY_OF_MONTH, Integer.parseInt(day));
					// System.out.println ("DATE=" + new String (fields[0]));
				}
			}
		}
		/*
		 * If the hour field wasn't set, but the year was, parse the hour from
		 * the year field.
		 */
		else if (Character.isDigit(fields[0][0])) {
			...
			if (hour.length() != 2) {
				...
				pt.set(PartialTimestamp.HOUR_OF_DAY, Integer.parseInt(hour));
		...
		if (Character.isDigit(fields[4][0])) {
			pt.set(PartialTimestamp.MINUTE, Integer.parseInt(new String(fields[4])));
		...
		if (Character.isDigit(fields[5][0])) {
			pt.set(PartialTimestamp.SECOND, Integer.parseInt(new String(fields[5])));
		...
		if ((fields[6][0] != ' ') && Character.isDigit(fields[7][0])
			...
			pt.set(PartialTimestamp.ZONE_OFFSET, offset);
			// System.out.println ("ZONE_OFFSET=" + offset);
		}
		/* If the offset is partially set, we have a problem. */
		else if (!((fields[6][0] == ' ') && !Character.isDigit(fields[7][0]) && !Character
				.isDigit(fields[8][0]))) {
