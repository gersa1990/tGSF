 ...
 *   DelegationSession session = factory.getSession ();
 *
 *   try {
 *      session.init (null);
...
public class PartialTimestamp extends Calendar {
   ...
   public void add (int field, int amount) {
      ...
      if (!isSet (field)) {
         ...
         /* If it's the year, and we know the year, pin the day if needed */
            ...
            int fullYear = this.getYear ();
            int daysInYear = getLengthOfYear (fullYear);
            ...
            daysInYear = getLengthOfYear (fullYear);
   ...
   private void adjustFields () {
      ...
      if (lastDayFieldSet == DAY_OF_MONTH) {
         ...
         if ((dayOfMonth < 1) ||
             (dayOfMonth > getLengthOfMonth (month, false))) {
            if (isLenient ()) {
      ...
      switch (field) {
         ...
         case DAY_OF_WEEK_IN_MONTH:
            if (isSet (MONTH) && isSet (YEAR) && isSet (CENTURY)) {
               ...
               if (calculateDayOfWeek (1, month, year) != SUNDAY) {
                  numWeeks++;
               }
               
               // If the month doesn't end on a SATURDAY, there's a partial
               // week at the end.
               if (calculateDayOfWeek (getLengthOfMonth (month, isLeapYear (year)),
                                            month, year) != SATURDAY) {
                  numWeeks++;
               }
