...
#os.system("python properiesGenerator.py -f C:\Users\Anuar\workspace\Experiments\Multiple_Machines_Week\ -n 30 -c teikoku_1_ -s {(1;7)} -m 2")

for j in range(len(NumberMachines)):
    for i in range(len(SLAStrings)):
        exectString = 'python properiesGenerator.py -f ' +'C:\Users\Anuar\workspace\Experiments\Multiple_Machines_Week\\'+ 'Machines_' + str(NumberMachines[j]) +'\\'+ 'SLA_' + SLALabels[i] +'\\ -n 30 -c teikoku_1_ -s {' + concString(SLAStrings[i],i) + '} -m ' + str(NumberMachines[j])
